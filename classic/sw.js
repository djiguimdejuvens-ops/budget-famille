const CACHE='budgetclassic-2026-08-08-18';
self.addEventListener('install',()=>self.skipWaiting());
self.addEventListener('activate',e=>e.waitUntil((async()=>{const ks=await caches.keys();await Promise.all(ks.filter(k=>k.startsWith('budgetclassic-')&&k!==CACHE).map(k=>caches.delete(k)));await self.clients.claim();})()));
