const CACHE='budgetv3-2026-08-08-38';
...
self.addEventListener('fetch',e=>{const r=e.request;if(r.method!=='GET')return;let u;try{u=new URL(r.url);}catch(_){return;}if(u.origin!==location.origin)return;e.respondWith((async()=>{try{const f=await fetch(r);const c=await caches.open(CACHE);c.put(r,f.clone());return f;}catch(err){const c=await caches.match(r);return c||caches.match('./index.html');}})());});
